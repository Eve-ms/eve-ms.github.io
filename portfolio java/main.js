...
json.forEach((item) => {
  ...
  aboutButton.addEventListener("click", () => {
    ...
    content.innerHTML = `<h2>Eve Mousain</h2>
    <br>
    <p>I'm a graphic design student at ESAD Amiens since 2023. I like to work on various projects, from design to photography, 3D, volume and also typography. I have been learning 3D by myself since a few years and I'm looking forward to incorporating it into some of my future projects.</p>`;
    wrap.appendChild(content);

    const close = document.createElement("div");
    close.textContent = "×";
    close.classList.add("close1");
  ...
  // Add hover animation for the "about" button
  aboutButton.addEventListener("mouseenter", () => {
    // Scale up the "about" button
    gsap.to(aboutButton, { scale: 2, duration: 0.3, ease: "power1.inOut" });

    // Dim and stop the movement of project divs
    allProjets.forEach((projet) => {
      gsap.to(projet, { opacity: 0.2, duration: 0.3 });
      gsap.killTweensOf(projet); // Stop floating animation
    });
  });

  aboutButton.addEventListener("mouseleave", () => {
    // Restore the "about" button scale
    gsap.to(aboutButton, { scale: 1, duration: 0.3, ease: "power1.inOut" });

    // Restore the opacity and floating animation of project divs
    allProjets.forEach((projet, index) => {
      gsap.to(projet, { opacity: 1, duration: 0.3 });
    ...
    allProjets.forEach((projet) => {
      const createTrail = () => {
        ...
        gsap.to(trail, { opacity: 0, duration: 0.1, onComplete: () => trail.remove() });
      };
...
//Animations projets//
  ...
  allProjets.forEach((projet) => {
    projet.addEventListener("mouseenter", () => {
      const rect = projet.getBoundingClientRect();
      const gradient = `radial-gradient(circle at ${rect.left + rect.width / 2}px ${rect.top + rect.height / 2}px, rgb(234, 255, 0) 0%, #750000 80%)`;
      gsap.to(dynamicBg, { background: gradient, duration: 0, ease: "power2.inOut" });
    });

    projet.addEventListener("mouseleave", () => {
      gsap.to(dynamicBg, { background: "#750000", duration: 0, ease: "power2.inOut" });
    });
  });
